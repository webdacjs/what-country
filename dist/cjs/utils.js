...
exports.getCurrencyField = exports.pickReturnFields = exports.getFilterField = exports.dedup = exports.toInt = exports.lower = void 0;
exports.checkDupsAltNames = checkDupsAltNames;
 ...
 * toInt(null)   // 0
 */
const toInt = (str) => parseInt(String(str ?? 0));
exports.toInt = toInt;
/**
 ...
 */
const dedup = (str) => Array.from(new Set(str.replace(/,/g, " ").split(" "))).join(" ");
exports.dedup = dedup;
/**
 ...
 */
const pickReturnFields = (results, fields) => !fields
    ? results
    : results.map((country) => {
        return fields.reduce((acc, field) => {
            if (field in country) {
                acc[field] = country[field];
            }
            return acc;
        }, {});
    });
exports.pickReturnFields = pickReturnFields;
/**
...
exports.getCurrencyField = getCurrencyField;
/**
 ...
 */
const testAltNames = (q) => altnames.get(q) ? countries.filter((x) => x.iso === altnames.get(q)) : [];
/**
 ...
 */
function checkDedupAltNames(dedupq, fields) {
    const filtered = testAltNames(dedupq);
    return filtered.length > 0 ? (0, exports.pickReturnFields)(filtered, fields) : [];
}
/**
 ...
 */
function checkAltNames(lq, dedupq, fields) {
    const filtered = testAltNames(lq);
    return filtered.length > 0
        ? (0, exports.pickReturnFields)(filtered, fields)
 ...
 */
function checkDupsAltNames(lq, fields) {
    const dedupq = (0, exports.dedup)(lq);
    const filtered = countries.filter((x) => (0, exports.lower)(x[(0, exports.getFilterField)(lq)]) === dedupq);
    return filtered.length > 0
        ? (0, exports.pickReturnFields)(filtered, fields)
