 ...
 * toInt(null)   // 0
 */
export const toInt = (str) => parseInt(String(str ?? 0));
/**
 ...
 */
export const dedup = (str) => Array.from(new Set(str.replace(/,/g, " ").split(" "))).join(" ");
/**
 ...
 */
export const pickReturnFields = (results, fields) => !fields
    ? results
    : results.map((country) => {
 ...
 */
export const getCurrencyField = (q) => q.length === 3 ? "currencycode" : "currencyname";
/**
 ...
 */
export function checkDupsAltNames(lq, fields) {
    const dedupq = dedup(lq);
    const filtered = countries.filter((x) => lower(x[getFilterField(lq)]) === dedupq);
    return filtered.length > 0
        ? pickReturnFields(filtered, fields)
