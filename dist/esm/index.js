import { lower, toInt, getFilterField, getCurrencyField, pickReturnFields, checkDupsAltNames, } from "./utils";
 ...
 */
export function queryCurrency(q, fields) {
    const lq = lower(q);
    const filtered = countries.filter((x) => lower(x[getCurrencyField(q)]) === lq);
 ...
 */
export function queryPhone(q, fields) {
 ...
 */
export function queryCitizenship(q, fields) {
    const lq = lower(q);
    const filtered = countries.filter((x) => x.nationality.map((nat) => lower(nat)).includes(lq));
 ...
 */
export function query(q, fields) {
    const lq = lower(q);
    const filtered = countries.filter((x) => lower(x[getFilterField(lq)]) === lq);
 ...
 */
export function queryfuzzy(q, fields) {
    const lq = lower(q);
