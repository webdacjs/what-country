 ...
 */
function sortByPopulationDesc(arr: Country[]): Country[] {
 ...
 */
export function queryCurrency(q: string, fields?: string[]): QueryResult {
  const lq = lower(q);
  const filtered = countries.filter(
    (x) => lower(x[getCurrencyField(q)] as string) === lq,
  );
  return pickReturnFields(filtered, fields);
}
 ...
 */
export function queryPhone(q: string, fields?: string[]): QueryResult {
  const intQuery = toInt(q);
  const filtered = countries.filter((x) => toInt(x.phone) === intQuery);
  return pickReturnFields(filtered, fields);
}
 ...
 */
export function queryCitizenship(q: string, fields?: string[]): QueryResult {
  const lq = lower(q);
  const filtered = countries.filter((x) =>
    x.nationality.map((nat) => lower(nat)).includes(lq),
  );
  const sorted =
    filtered.length > 0 ? sortByPopulationDesc(filtered) : filtered;
  return pickReturnFields(sorted, fields);
}
 ...
 */
export function query(q: string, fields?: string[]): QueryResult {
  const lq = lower(q);
  const filtered = countries.filter(
    (x) => lower(x[getFilterField(lq)] as string) === lq,
  );
  return filtered.length > 0
    ? pickReturnFields(filtered, fields)
    : checkDupsAltNames(lq, fields);
}
 ...
 */
export function queryfuzzy(q: string, fields?: string[]): QueryResult {
  const lq = lower(q);
  const filtered = countries.filter((x) => lq.includes(lower(x.name)));
  const sorted =
    filtered.length > 0 ? sortByPopulationDesc(filtered) : filtered;
  return pickReturnFields(sorted, fields);
}
